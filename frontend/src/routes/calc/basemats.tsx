...
import { items, type ItemName } from "../../consts/items";
import { posboms, type BaseBom, type StructureName } from "../../consts/structures";
...
function CargoCalculator() {
  ...
  const [cargoVolume, setCargoVolume] = useSessionStorage<number>("cargoVolume", 0);
  ...
  useEffect(() => {
    ...
    for (const [pos, posCount] of Object.entries(baseBom) as [StructureName, number][]) {
      ...
      for (const [item, itemCount] of Object.entries(posboms[pos].components)) {
